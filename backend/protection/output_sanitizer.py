...
# FIX: was `from detection.exfiltration` — correct path is relative or backend-prefixed
from backend.detection.exfiltration import ExfiltrationDetector


def _redact_pii_matches(text: str, pii_result) -> str:
    """
    FIX: PIIResult has no `redacted_preview` field.
    It has `matches: dict[str, list[str]]` — manually replace each matched value.
    """
    result = text
    for pii_type, found_list in pii_result.matches.items():
        for match_str in found_list:
            if match_str and match_str in result:
                result = result.replace(match_str, f"[REDACTED_{pii_type.upper()}]")
    return result
...
class OutputSanitizer:
    ...
    def sanitize(self, output: str, agent_id: str) -> tuple[str, list]:
        ...
        # 1. Remove PII — FIX: use _redact_pii_matches() instead of .redacted_preview
        ...
        if pii_result.found_types:
            ...
            sanitized = _redact_pii_matches(sanitized, pii_result)
